...
pub fn handler_get_public_key(comm: &mut Comm, display: bool) -> Result<(), AppSW> {
    ...
    let pk_le = with_derived_key(path.as_ref(), |sk| {
        let pk_comp = xelis_public_from_private(sk.as_ref()).map_err(|_| AppSW::KeyDeriveFail)?;
    ...
    if display {
        ...
        if !ui_display_pk(&addr_bytes[..len])? { return Err(AppSW::Deny); }
    }
    comm.append(&[32u8]);
    comm.append(&pk_le);
    Ok(())
