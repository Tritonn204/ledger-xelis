use crate::{
    crypto::{address::*, public_key::*, ristretto::*, secure::*, *},
    AppSW,
};
...
pub fn handler_debug_keys(comm: &mut Comm) -> Result<(), AppSW> {
    ...
    match apdu_header.p2 {
        ...
        1 => handler_get_generator_full(comm),
...
fn test_public_key_derivation(comm: &mut Comm) -> Result<(), AppSW> {
    ...
    match xelis_public_from_private(&private_2) {
        Ok(public_compressed) => {
            ...
            let expected_le = [
                0xf0, 0x5b, 0xc1, 0xdf, 0x28, 0x31, 0x71, 0x7c, 0x29, 0x92, 0xd8, 0x5b, 0x57, 0xe0,
                0xcf, 0x3d, 0x12, 0x3f, 0xd6, 0xc2, 0x54, 0x25, 0x7d, 0xe5, 0xf7, 0x84, 0xbe, 0x36,
                0x97, 0x47, 0xb2, 0x49,
            ];
    ...
    match xelis_public_from_private(&private_3_be) {
        Ok(public_compressed) => {
            ...
            let expected_le = [
                0x02, 0x06, 0x4b, 0x89, 0xdc, 0x89, 0xf5, 0xc3, 0x53, 0xcf, 0x20, 0x77, 0x80, 0x0e,
                0x24, 0xfb, 0x83, 0x30, 0x0d, 0x48, 0xb1, 0xaf, 0x4a, 0x39, 0x26, 0xf1, 0xfe, 0x0a,
                0x18, 0x64, 0xcf, 0x06,
            ];
    ...
    match xelis_public_from_private(&private_4_be) {
        Ok(public_compressed) => {
            ...
            let expected_le = [
                0xf6, 0xde, 0xcf, 0xbf, 0x9e, 0xfa, 0xbc, 0x8a, 0xa5, 0x94, 0x52, 0xaa, 0x57, 0x0c,
                0xb8, 0x4e, 0xed, 0x7f, 0xcf, 0xca, 0x7d, 0xae, 0xa5, 0x8a, 0x93, 0xb9, 0x34, 0x44,
                0x40, 0x0a, 0x7a, 0x73,
            ];
...
fn test_address_generation(comm: &mut Comm) -> Result<(), AppSW> {
    ...
    comm.append(&[0xB2]);
    let public_2_bytes = [
        0xf0, 0x5b, 0xc1, 0xdf, 0x28, 0x31, 0x71, 0x7c, 0x29, 0x92, 0xd8, 0x5b, 0x57, 0xe0, 0xcf,
        0x3d, 0x12, 0x3f, 0xd6, 0xc2, 0x54, 0x25, 0x7d, 0xe5, 0xf7, 0x84, 0xbe, 0x36, 0x97, 0x47,
        0xb2, 0x49,
    ];
    ...
    comm.append(&[0xB3]);
    let public_3_bytes = [
        0x02, 0x06, 0x4b, 0x89, 0xdc, 0x89, 0xf5, 0xc3, 0x53, 0xcf, 0x20, 0x77, 0x80, 0x0e, 0x24,
        0xfb, 0x83, 0x30, 0x0d, 0x48, 0xb1, 0xaf, 0x4a, 0x39, 0x26, 0xf1, 0xfe, 0x0a, 0x18, 0x64,
        0xcf, 0x06,
    ];
    ...
    comm.append(&[0xB4]);
    let public_4_bytes = [
        0xf6, 0xde, 0xcf, 0xbf, 0x9e, 0xfa, 0xbc, 0x8a, 0xa5, 0x94, 0x52, 0xaa, 0x57, 0x0c, 0xb8,
        0x4e, 0xed, 0x7f, 0xcf, 0xca, 0x7d, 0xae, 0xa5, 0x8a, 0x93, 0xb9, 0x34, 0x44, 0x40, 0x0a,
        0x7a, 0x73,
    ];
...
pub fn handler_get_generator_full(comm: &mut Comm) -> Result<(), AppSW> {
    pub const XELIS_H_COMPRESSED: [u8; 32] = [
        0x34, 0x11, 0x87, 0x48, 0x40, 0xf3, 0x46, 0x7b, 0xcd, 0xcb, 0x44, 0x7f, 0xdb, 0x8c, 0xa0,
        0x94, 0x5f, 0x74, 0x8d, 0x04, 0xa1, 0x77, 0xc3, 0x65, 0xdc, 0xe6, 0xa9, 0x56, 0xb4, 0x40,
        0x92, 0x8c,
    ];

    pub const XELIS_G_COMPRESSED: [u8; 32] = [
        0x76, 0x2d, 0x8d, 0xe0, 0x45, 0x59, 0xa6, 0xb6, 0x8d, 0xdd, 0x82, 0xa5, 0x6a, 0x0b, 0xe3,
        0x58, 0x5f, 0x51, 0x00, 0xc5, 0x61, 0xa9, 0x84, 0xa8, 0x71, 0x4e, 0xbc, 0x6a, 0x0a, 0xae,
        0xf2, 0xe2,
    ];
