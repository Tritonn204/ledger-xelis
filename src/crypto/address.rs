...
pub fn convert_bits_fixed(data: &[u8], from: u8, to: u8, pad: bool, output: &mut [u8]) -> Result<usize, AppSW> {
    let mut acc: u32 = 0;
    let mut bits: u8 = 0;
    let mut idx = 0;
...
fn shorten_addr_with_prefix(addr: &str, keep_left: usize, keep_right: usize) -> String {
    if let Some(colon) = addr.find(':') {
        ...
        format!("{}{}...{}", hrp_with_sep, &data[..keep_left], &data[data.len() - keep_right..])
    } else {
